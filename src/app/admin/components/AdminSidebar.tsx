...
export function AdminSidebar({ categories, unreadCount }: AdminSidebarProps) {
    ...
    const navItemClass = (path: string) =>
        `group flex items-center gap-3 px-4 py-3 rounded-xl text-[11px] uppercase tracking-[0.12em] transition-all duration-300 relative overflow-hidden ${isActive(path)
            ? 'bg-gradient-to-r from-gold-400/15 to-gold-400/5 text-gold-400 border border-gold-400/20 shadow-[0_0_20px_rgba(212,175,55,0.05)]'
    ...
    const sidebar = (
        <div className="flex flex-col h-full">
                        ...
                        {categories.length > 0 && (
                            <>
                                <button
                                    ...
                                    className="flex items-center gap-3 px-4 py-2 w-full text-[9px] uppercase tracking-[0.12em] text-gray-600 hover:text-gray-400 transition-colors rounded-lg"
                                >
                                    ...
                                    <div className="ml-4 pl-4 border-l border-white/[0.04] space-y-0.5">
                                        {categories.filter(c => !c.parent_id).map(cat => (
                                            <Link
                                                ...
                                                className="block px-3 py-1.5 text-[9px] uppercase tracking-[0.1em] rounded-lg transition-colors text-gray-600 hover:text-gold-400 hover:bg-gold-400/5"
                                                onClick={() => setMobileOpen(false)}
                                            >
                                                {cat.name}
                                            </Link>
            ...
            <div className="p-4 border-t border-white/[0.04]">
                <Link
                    href="/"
                    className="flex items-center gap-3 px-4 py-3 rounded-xl text-[10px] uppercase tracking-[0.12em] text-gray-500 hover:text-gold-400 hover:bg-gold-400/5 transition-all duration-300 border border-transparent hover:border-gold-400/10"
                    target="_blank"
                >
